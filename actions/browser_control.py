...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".Athena_profiles" / browser
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".Athena_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using Athena profile")
                Athena = str(Path.home() / ".Athena_profiles" / "firefox_Athena")
                Path(Athena).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(Athena, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".Athena_profiles" / "safari")
        ...
        # Athena otomasyon profiline geçilir — buraya bir kez giriş yapılan
        # hesaplar sonraki oturumlarda da açık kalır.
        Athena_profile = str(Path.home() / ".Athena_profiles" / self.browser_name)
        Path(Athena_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with Athena profile: {Athena_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(Athena_profile, **kwargs)
            self._page = await self._adopt_page()
            print(f"[Browser] ✅ Launched [{label}] with Athena profile "
    ...
    async def _get_page(self) -> Page:
        ...
        await self._launch()
        if self._context is None:
            raise RuntimeError(f"No browser context for '{self.browser_name}'.")
        # If somehow page got closed, open a fresh one
        if self._page is None or self._page.is_closed():
            try:
                self._page = await self._context.new_page()
            except Exception as e:
                # Context died mid-flight — relaunch once
                print(f"[Browser] new_page failed ({e}); relaunching context")
                await self._reset_context()
                await self._launch()
                if self._context is None:
                    raise
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "Athena_screenshot.png")
...
def browser_control(
    ...
    # ── Navigation ALWAYS prefers the user's real browser ───────────────────
    # go_to / search / new_tab open in the user's own browser (real profile).
    # Only reuse an existing automation session if it is still healthy; if the
    # Playwright context has died ("Target page/context closed"), drop it and
    # fall back to a native OS launch so simple Google searches keep working.
    if action in ("go_to", "search", "new_tab"):
        if _registry.has(browser):
            ...
            try:
                ...
                err = str(e)
                print(f"[Browser] automation path failed ({action}): {err}")
                # Stale/dead CDP session — discard and open natively instead
                try:
                    target = (browser or (sess.browser_name if sess else "")
                    ...
                    print(f"[Browser] could not drop dead session: {close_err}")
                # fall through to native open below
    ...
    # These need a controllable browser; open an automation window only here.
    try:
        sess = _registry.get(browser)
    except Exception as e:
    ...
    try:
        ...
        err = str(e)
        # Dead context mid-action — drop session so the next call can relaunch
        if "Target page" in err or "has been closed" in err or "Target closed" in err:
            try:
                _registry.close_one(sess.browser_name)
            except Exception:
                pass
            result = (
                f"Browser session was closed unexpectedly ({action}). "
                f"Ask me again and I will reopen it."
            )
        else:
