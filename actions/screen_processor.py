...
def _assistant_name() -> str:
    return _load_config().get("assistant_name") or "Athena"


def _system_prompt() -> str:
    name = _assistant_name()
    return (
        f"You are {name}, a professional AI assistant. "
        ...
        "A brief natural reaction is fine, then the useful detail. "
        ...
        while True:
            try:
                ...
                async with client.aio.live.connect(
                    model=get_live_model(), config=config
                ) as session:
            ...
            async for response in self._session.receive():
                ...
                if sc.turn_complete:
                    if transcript and self._player:
                        ...
                        if full:
                            self._player.write_log(f"{_assistant_name()}: {full}")
                            print(f"[Vision] 💬 {full}")
                    transcript = []
                    # Auto-close camera ~2s after Athena finishes speaking
