...
class EdgeTTSEngine:
    ...
    async def _synth(self, text: str) -> bytes:
        try:
            ...
            raise RuntimeError(
                "edge_tts is not installed. Run: pip install edge-tts"
            ) from e
    ...
    def synthesize(self, text: str) -> tuple[bytes, str]:
        ...
        try:
            ...
            loop.close()
        return audio_bytes or b"", ".mp3"

    def speak(self, text: str) -> None:
        audio_bytes, _ext = self.synthesize(text)
        if audio_bytes:
...
class TTSPlayer:
    ...
    def synthesize_to_file(self, text: str, dest=None):
        from pathlib import Path
        import tempfile
        import time

        if not hasattr(self._engine, "synthesize"):
            raise RuntimeError("This TTS engine cannot synthesize to a file.")
        data, ext = self._engine.synthesize(text)
        if not data:
            raise RuntimeError("TTS produced no audio.")
        path = Path(dest) if dest else Path(tempfile.gettempdir()) / (
            f"athena_wa_voice_{int(time.time() * 1000)}{ext}"
        )
...
_last_player: TTSPlayer | None = None
...
def create_tts_player(config: dict) -> TTSPlayer:
    ...
    _last_player = TTSPlayer(engine)
    return _last_player


def synthesize_to_file(text: str, dest=None):
    """Synthesize speech to a temp audio file without playing it."""
    player = _last_player
    if player is None:
        try:
            ...
            config = {}
        player = create_tts_player(config if isinstance(config, dict) else {})
    return player.synthesize_to_file(text, dest)
